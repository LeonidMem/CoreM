...
execute unless block ~ ~ ~ barrel{Items:[{Slot:15b,tag:{Tags:[corem.clear]}}]} run kill @e[type=item,nbt={Item:{id:"minecraft:barrier",tag:{Tags:[corem.clear]}}}]

# I don't really know what does it do, but maybe it removes some bugs or dupes...
execute unless data block ~ ~ ~ Items[{Slot:15b,tag:{Tags:[corem.clear]}}] positioned ~ ~1 ~ run function corem:custom_wb/anti_barrier
